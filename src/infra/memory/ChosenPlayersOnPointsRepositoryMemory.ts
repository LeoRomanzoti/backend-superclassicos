...
export default class ChosenPlayerOnPointsRespositoryMemory implements ChosenPlayerOnPointsRespository {
    ...
    async save(pointId: string, chosenPlayerId: string): Promise<boolean> {
        const newPoint = new ChosenPlayersOnPointsDTO("7", new Point(pointId, 8, "Gol", "G"))
        this.chosenPlyersOnPoints.push(newPoint)
        return true
    }

    async getByChosenPlayerId(chosenPlayerId: string): Promise<ChosenPlayersOnPointsDTO[]> {
