...
export default class ChosenPlayersOnPointsController {
    ...
    async addPoint(
        ...
        const chosenPlayersOnPointsRepository =
            new ChosenPlayersOnPointsRepositoryDatabase(
                this.databaseConnection
            );
        const useCase = new AddPointToChosenPlayer(
            chosenPlayersOnPointsRepository,
            chosenPlayerRepository,
            pointRepository
        );
        const chosenPlayerWithUpdatedPoint = useCase.execute(
            pointId,
            chosenPlayerId
        );
    ...
    async getAllByChosenPlayerId(
        ...
        const chosenPlayerPoints =
            await chosenPlayersOnPointsRepository.getByChosenPlayerId(
                chosenPlayerId
            );
